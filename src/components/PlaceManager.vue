        ...
        <el-tabs v-model="activeTagName" type="card">
          <el-tab-pane label="资源占用表" name="resTable">
            <el-table
              ...
              <el-table-column prop="pmsAllocateResource.arResAmount" label="占用面积" align="center" width="110"></el-table-column>
  ...
  methods: {
    ...
    async getPlaceList() {
      ...
      if (this.placeList.length > 0) {
        this.currPlace = this.placeList[0]
        this.getArList()
      }
    },
    // 根据组织UID请求场地列表
    ...
    // 请求资源占用情况列表
    async getArList() {
      const { data: res } = await this.$http.get('/ar/getAllocateResourceListByResUid/' + this.currPlace.placeUid)
      for (let i = 0, len = res.length; i < len; i++) {
        res[i].arResName = this.currPlace.placeName
        res[i].arResId = this.currPlace.placeId
      }
      this.arList = res
    },
    // 请求项目列表数据
    ...
    arInsert() {
      ...
      this.taskList = []
      // 默认资源名称及编号
      this.arForm.arResName = this.currPlace.placeName
      this.arForm.arResId = this.currPlace.placeId
      this.arForm.pmsAllocateResource.arResUid = this.currPlace.placeUid
